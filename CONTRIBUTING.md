...
动态任务状态与角色工作流以 `docs/TASK_DISPATCH.md` 为唯一入口。本文件只定义稳定的分支和 PR 流程。

## 开始条件

只有在 `docs/TASK_DISPATCH.md` 中标记为 `READY` 的 Issue 才能首次派发。已是 `IN_PROGRESS` 或 `CHANGES_REQUESTED` 的任务只能由原负责人继续。
...
1. 高阶模型确认 Issue 契约完整、依赖已合并，并在协作控制中心标记 `READY`。
...
7. 高阶模型确认 head、CI、验收证据和线程后给出结论。
8. 通过后由仓库所有者或其明确授权者 squash merge，并删除任务分支。
9. 高阶模型在合并后更新 `docs/TASK_DISPATCH.md`，解锁下游任务。
...
- `docs/TASK_DISPATCH.md` 保存当前状态和依赖；由高阶模型在 GitHub 状态变化后更新。
- README 只提供入口和当前阶段摘要；ROADMAP 只维护里程碑。
- 规则变化更新 `docs/GAME_RULES.md`；架构变化通过 ADR 并同步 `docs/ARCHITECTURE.md`。
- 低阶执行模型不得在未授权时修改协作控制中心。
